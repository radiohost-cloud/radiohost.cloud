...
const MediaLibrary: React.FC<MediaLibraryProps> = ({ rootFolder, onAddToPlaylist, onAddUrlTrackToLibrary, onRemoveFromLibrary, onCreateFolder, onMoveItem, onOpenMetadataSettings, onOpenTrackMetadataEditor, onUpdateTrackTags, onUpdateFolderTags, onPflTrack, pflTrackId, playoutMode }) => {
    ...
    const processFiles = useCallback(async (files: File[]) => {
        ...
        for (const [index, file] of files.entries()) {
            ...
            try {
                // Always get duration from the client-side, as it's more reliable.
                const duration = await getAudioDuration(file, file.name);
    
                if (isHostMode) {
                    // In HOST mode, send the reliable duration to the server.
                    ...
                    // In DEMO mode, process everything in the browser.
                    ...
                    // Also fetch remote artwork for DEMO mode if needed
                    if (!picture) {
                        newTrack.remoteArtworkUrl = await fetchArtwork(newTrack.artist!, newTrack.title) ?? undefined;
                    }
                    
                    let artworkBlob: Blob | undefined = undefined;
                    if (picture) {
                        artworkBlob = new Blob([new Uint8Array(picture.data)], { type: picture.format });
                    }
    
                    await dataService.addTrack(newTrack, file, artworkBlob);
                }
            } catch (error) {
