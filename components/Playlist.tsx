            ...
            <div className="flex-grow flex items-center gap-4 overflow-hidden">
                ...
                <div className="truncate flex items-center gap-2">
                     <p className={`font-medium truncate ${textColorClass}`}>
                        {track.type === TrackType.VOICETRACK && <VoiceTrackIcon className="inline-block w-4 h-4 mr-2" />}
                        {displayText}
                     </p>
    ...
    return (
        ...
        >
                ...
                <ul className="p-2 space-y-1">
                                            ...
                                            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2">
                                                {showAddMarkerButton && (
                                                     <button 
                                                        ...
                                                        className="p-1 bg-neutral-100 dark:bg-neutral-900 rounded-full text-neutral-400 dark:text-neutral-600 hover:bg-red-500 hover:text-white scale-90 group-hover:scale-100 opacity-70 group-hover:opacity-100 transition-all"
                                                        title="insert time marker here"
                                                    >
                                                ...
                                                {showAddVtButton && (
                                                    <button 
                                                        onClick={() => setVtEditorState({ isOpen: true, prevTrack: (prevItem && !('markerType' in prevItem) ? prevItem : null), nextTrack: item, beforeItemId: item.id })}
                                                        className="p-1 bg-neutral-100 dark:bg-neutral-900 rounded-full text-neutral-400 dark:text-neutral-600 hover:bg-red-500 hover:text-white scale-90 group-hover:scale-100 opacity-70 group-hover:opacity-100 transition-all"
                                                        title="insert voicetrack here"
                                                    >
                                                        <VoiceTrackIcon className="w-5 h-5"/>
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                     ...
                     {(isMarkerModeActive || isVtModeActive) && !isContributor && (items.length === 0 || !('markerType' in items[items.length - 1])) && (
                        <li className="flex justify-center items-center h-4 my-1 group">
                            <div className="w-full h-px bg-neutral-200 dark:bg-neutral-800 relative">
                                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2">
                                    {isMarkerModeActive && (items.length === 0 || (!('markerType' in items[items.length - 1]) && items[items.length - 1].type !== TrackType.VOICETRACK)) && (
                                        <button
                                    ...
                                    {isVtModeActive && items.length > 0 && !('markerType' in items[items.length - 1]) && items[items.length - 1].type !== TrackType.VOICETRACK && (
                                         <button 
                                            onClick={() => setVtEditorState({ isOpen: true, prevTrack: (items[items.length - 1] as Track), nextTrack: null, beforeItemId: null })}
                                            className="p-1 bg-neutral-100 dark:bg-neutral-900 rounded-full text-neutral-400 dark:text-neutral-600 hover:bg-red-500 hover:text-white scale-90 group-hover:scale-100 opacity-70 group-hover:opacity-100 transition-all"
                                            title="insert voicetrack here"
                                        >
                                            <VoiceTrackIcon className="w-5 h-5"/>
            ...
            <ConfirmationDialog
                ...
                title={`Remove ${itemToDelete && 'markerType' in itemToDelete ? 'Marker' : 'Track'}`}
            >
                Are you sure you want to remove "{itemToDelete && !('markerType' in itemToDelete) ? itemToDelete.title : 'Time Marker'}" from the playlist?
            </ConfirmationDialog>
             <AddTimeMarkerModal
            ...
            <VoiceTrackEditor
                ...
                onSave={async (vtData) => {
                    await onInsertVoiceTrack(vtData.track, vtData.blob, vtData.vtMix, vtEditorState.beforeItemId);
                }}
