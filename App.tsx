...
const AppInternal: React.FC = () => {
    ...
    const [activeRightColumnTab, setActiveRightColumnTab] = useState<'cartwall' | 'ai-playlist' | 'ai' | 'mixer' | 'settings' | 'scheduler' | 'stream'>('cartwall');
    ...
    useDebouncedEffect(() => {
        ...
        if (isContributor) {
            ...
            dataService.putUserData(key, masterData);
    ...
    const handlePlayTrack = useCallback(async (itemId: string) => {
        ...
        setIsPlaying(true);
    }, [stopPfl, handleSetCurrentTrack, playoutPolicy.playoutMode]);
    ...
    }, [activePlayer, getTrackSrc, stopPfl, handleSetCurrentTrack]);
                ...
                try {
                    ...
                    if (track && !('markerType' in track) && track.addedBy === 'broadcast') {
                        ...
                        if (!previousItem || 'markerType' in previousItem || (!('markerType' in previousItem) && previousItem.addedBy !== 'broadcast')) {
                             ...
                             setPlaylist(currentList => currentList.slice(currentIndex));
                             setCurrentTrackIndex(0);
                        }
                    }
            ...
            const nowPlayingIndex = playlist.findIndex(item => item.id === currentPlayingItemId);
             if (nowPlayingIndex > -1) {
                ...
                for (let i = 0; i < nowPlayingIndex; i++) {
                    const item = playlist[i];
                    if (!('markerType' in item) && !softSkippedIds.has(item.id)) {
                        const nextHardMarkerIndex = playlist.findIndex((nextItem, index) => index > i && 'markerType' in nextItem && nextItem.markerType === TimeMarkerType.HARD);
    ...
    useEffect(() => {
        ...
        const handleEnded = (e: Event) => {
            ...
            if (nextIndex !== -1) {
                ...
                setIsPlaying(false);
                setCurrentPlayingItemId(null);
                if (playoutPolicyRef.current.removePlayedTracks) setPlaylist([]);
            }
        };
    ...
    }, [activePlayer, findNextPlayableIndex, performCrossfade, handleSetCurrentTrack]);
    ...
    const handleMicStream = useCallback((stream: MediaStream | null) => {
        ...
        if (graph.isInitialized && graph.context) {
            if (graph.sources.mic) graph.sources.mic.disconnect();
            if (stream) {
                try {
                ...
                } catch (e) { console.error("Error creating mic audio source:", e); }
            } else {
    ...
    const handleAddGeneratedPlaylist = useCallback((tracks: Track[]) => {
        setPlaylist(prev => [...prev, ...tracks.map(t => ({...t, addedBy: 'user'} as SequenceItem))]);
    }, []);
    ...
    const handleInsertTrackInPlaylist = useCallback((track: Track, beforeItemId: string | null) => {
        setPlaylist(prev => {
            ...
            if (insertIndex !== -1) newPlaylist.splice(insertIndex, 0, { ...track, addedBy: 'user' });
            else newPlaylist.push({ ...track, addedBy: 'user' });
    ...
    const handleInsertVoiceTrack = useCallback(async (voiceTrack: Track, blob: Blob, vtMix: VtMixDetails, beforeItemId: string | null) => {
        ...
        setPlaylist(prev => {
            ...
            if (insertIndex !== -1) newPlaylist.splice(insertIndex, 0, trackWithMix);
            else newPlaylist.push(trackWithMix);
            if (currentPlayingItemIdRef.current) {
                 const newCurrentIndex = newPlaylist.findIndex(item => item.id === currentPlayingItemIdRef.current);
                 if (newCurrentIndex !== -1) setCurrentTrackIndex(newCurrentIndex);
            }
    ...
    const handleRemoveFromLibrary = useCallback(async (id: string) => {
        ...
        await dataService.deleteArtwork(id);
    ...
    const handleUpdateFolderMetadataSettings = useCallback((folderId: string, settings: { enabled: boolean; customText?: string; suppressDuplicateWarning?: boolean }) => {
        setMediaLibrary(prevLibrary => updateFolderInTree(prevLibrary, folderId, folder => ({ ...folder, suppressMetadata: settings })));
    }, []);
    ...
    const handleUpdateTrackMetadata = useCallback((trackId: string, newMetadata: { title: string; artist: string; type: TrackType; remoteArtworkUrl?: string; }) => {
        setMediaLibrary(prevLibrary => updateTrackInTree(prevLibrary, trackId, track => ({ ...track, ...newMetadata })));
    }, []);

    const handleUpdateTrackTags = useCallback((trackId: string, tags: string[]) => {
        setMediaLibrary(prevLibrary => updateTrackInTree(prevLibrary, trackId, track => ({ ...track, tags: tags.length > 0 ? tags.sort() : undefined })));
    }, []);
...
const findFolderInTree = (node: Folder, folderId: string): Folder | null => {
    ...
    const handleUpdateFolderTags = useCallback((folderId: string, newTags: string[]) => {
        ...
        if (tagsToAdd.length === 0 && tagsToRemove.length === 0) {
            setMediaLibrary(prevLibrary => updateFolderInTree(prevLibrary, folderId, folder => ({ ...folder, tags: newTags.length > 0 ? newTags.sort() : undefined })));
            return;
        }
    ...
    const handleLogin = useCallback((email: string) => { window.location.reload(); }, []);
    const handleSignup = useCallback((email: string) => { window.location.reload(); }, []);
    ...
    const handleImportData = useCallback((data: any) => {
        try {
            if (data.library) setMediaLibrary(data.library);
